...
TypeScript 借助旧类型创建一个新类型的方式，就是映射类型，它可以用相同的形式去转换旧类型中的每个属性。

### 基础

+ Readonly: 适用于将一个对象中的每一个属性转换为**只读**的场景
...
+ Partial: 适用于将一个对象中的每一个属性转换为**可选**的场景
...
+ Record: 适用于将一个对象中的每一个属性转换为**其他值**的场景
