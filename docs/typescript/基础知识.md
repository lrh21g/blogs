...
接口是为类型命名和代码或者第三方代码定义契约。

在定义接口的时候，`{}` 括号包裹的是一个代码块，是声明类型的语句。
...
一个对象字面量存在任何“目标类型”不包含的属性时，会得到一个错误，主流的解决方法有：

+ 类型断言

  有点像一种类型转换，把某个值强行指定为特定类型。
...
可索引类型具有一个索引签名，它描述了对象索引的类型，和通过索引得到的返回值类型。
...
// 两人信息中 phone 属性有共同之处，key 都是 string 类型，value 都是 string 类型
// 此时，可以使用可索引类型表示，可索引类型具有一个索引签名
