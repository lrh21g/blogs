...
+ 单线程 : JavaScript 在浏览器中主要用于与用户互动、操作 DOM 。如果是多线程需要处理很多复杂的问题，比如两个现场同时操作 DOM ，一个线程删除当前 DOM 节点，一个线程操作当前 DOM 节点，不知道以哪个线程为准。为了避免这种情况，所以 JavaScript 是单线程的。虽然， H5 提出了web worker 标准，但是有很多限制，受主线程控制，是主线程的子线程。
...
+ 在 `vm._update()` 渲染真实 DOM过程中，执行 `vm.__patch__`，调用 `patch(oldVnode, vnode, hydrating, removeOnly)` 函数，将 VNode 转换成真实 DOM 节点
...
+ 删除旧节点：把 `oldVnode` 从当前 DOM 树中删除。如果父节点存在，则执行 `removeVnodes` 方法。
  ...
  组件的 `beforeDestroy` 和 `destroyed` 生命周期，就是在执行 `invokeDestroyHook` 过程中，执行了 `vnode` 的 `destory` 钩子函数（其定义在 `src\core\vdom\create-component.ts` 模块的 `componentVNodeHooks` 常量中）。
...
![observe](../files/images/observe.drawio.png)
